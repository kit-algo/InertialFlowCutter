...
#include "flow_cutter_accelerated.h"
...
namespace flow_cutter{
	...
	namespace expanded_graph{
		...
		inline int
		expanded_inter_arc_to_original_arc(int x, int original_arc_count){ (void)original_arc_count; return x/2; }
		inline int original_arc_to_expanded_inter_arc(int x, bool tail_out_flag, int original_arc_count){
		...
		template<class Tail, class Head, class ExpandedGraph, class FlowCutter>
		Separator extract_original_separator_from_cut(const Tail& original_tail, const Head& original_head, const ExpandedGraph& expanded_graph, const FlowCutter& cutter) {
			int original_node_count = original_tail.image_count();
			int original_arc_count = original_tail.preimage_count();
			(void) original_head;
			...
			for(auto x : cutter.get_current_cut()){
				if(is_expanded_inter_arc(x, original_arc_count)){
					auto l_exp = expanded_graph.tail(x), r_exp = expanded_graph.head(x);
					auto l = expanded_node_to_original_node(l_exp), r = expanded_node_to_original_node(r_exp);
		...
		std::vector<flow_cutter_accelerated::TerminalPair> expand_source_target_pair_list(std::vector<flow_cutter_accelerated::TerminalPair> p) {
			for (auto& x : p) {
				for (int& u : x.source_list) {
					u = original_node_to_expanded_node(u, false);
				}
				for (int& u : x.target_list) {
					u = original_node_to_expanded_node(u, true);
				}
			}
			return p;
		}
		...
		flow_cutter_accelerated::MultiCutter::TerminalInformation expand_source_target_pair_list(flow_cutter_accelerated::MultiCutter::TerminalInformation t) {
		    auto expand_order = [&](const std::vector<int>& order) {
		        ...
		        for (int i = 0; i < (int)order.size(); ++i) {
		           new_order[original_node_to_expanded_node(i, false)] = original_node_to_expanded_node(order[i], false);
		           new_order[original_node_to_expanded_node(i, true)] = original_node_to_expanded_node(order[i], true);
		        }
		        return new_order;
		    };
		    for (auto& te : t) {
		        te.node_order = expand_order(te.node_order);
		        if (te.build_node_order_from_distance) {
		        	te.st.source = original_node_to_expanded_node(te.st.source, false);
		        	te.st.target = original_node_to_expanded_node(te.st.target, true);
		        }
		    }
		    return t;
		}
