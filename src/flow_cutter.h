...
namespace flow_cutter{
	...
	public:
		...
		template<class Graph>
		void set_extra_node(const Graph&, int x){
			assert(!inside_flag(x));
			assert(extra_node == -1);
			inside_flag.set(x, true);
			++node_count_inside_;
		...
		int select_pierce_node(const Graph&graph, int side, const ScorePierceNode&score_pierce_node){
			...
			for(auto xy : assimilated[side].get_cut_front()){
				...
				if(!assimilated[1-side].is_inside(y)){
					auto score = score_pierce_node(y, side, reachable[1-side].is_inside(y), graph.arc_weight(xy));
	...
	public:
		template<class Graph>
		MultiCutter(const Graph& graph) : tmp(graph.node_count()) {}

		template<class Graph, class SearchAlgorithm,  class ScorePierceNode>
		void init(
			const Graph&graph,
			const SearchAlgorithm&search_algo, const ScorePierceNode&score_pierce_node, DistanceType dist_type, 
			const std::vector<SourceTargetPair>&p, int random_seed, bool should_skip_non_maximum_sides = true, bool parallel_ = false
		){
			parallel = parallel_;
			...
			auto init_cutter = [&](int i) {
				...
				x.init(graph, tmp.local(), search_algo, dist_type, p[i], random_seed + 1 + i);
				if (should_skip_non_maximum_sides)
					while (!x.does_next_advance_increase_cut(graph, my_score_pierce_node))
						x.advance(graph, tmp.local(), search_algo, my_score_pierce_node);
			};

			if (parallel) {
				tbb::parallel_for(0, (int)p.size(), init_cutter);
			}
			else {
		...
		template<class Graph, class SearchAlgorithm, class ScorePierceNode>
		bool advance(const Graph&graph, const SearchAlgorithm&search_algo, const ScorePierceNode&score_pierce_node, bool should_skip_non_maximum_sides = true){
			...
			for(;;){
				...
				auto handle_cutter = [&](int i) {
					...
					if(x.is_cut_available()){
						if((int)x.get_current_cut().size() == current_cut_size){
							...
							if(x.advance(graph, tmp.local(), search_algo, my_score_pierce_node)){
				...
				if (parallel) {
					tbb::parallel_for(0, (int)cutter_list.size(), handle_cutter);
				}
				else {
	...
	struct PierceNodeScore{
		...
		std::tuple<bool, int, int> operator()(int x, int side, bool causes_augmenting_path, int arc_weight, int source_dist, int target_dist, int)const{
			...
			case Config::PierceRating::max_target_minus_source_hop_dist_with_source_dist_tie_break:
				score = target_dist - source_dist;
				secondary_score = source_dist;
				break;
			case Config::PierceRating::max_target_minus_source_hop_dist_with_closer_dist_tie_break:
				score = target_dist - source_dist;
	...
	public:
		SimpleCutter(const Graph&graph, Config config):
			graph(graph), cutter(graph), config(config){
		}

		template<class GeoPos>
		void init(const std::vector<SourceTargetPair>&p, int random_seed, const GeoPos& /*geo_pos*/) { init(p, random_seed); }

		void init(const std::vector<SourceTargetPair>&p, int random_seed){
			bool parallel = config.thread_count > 1;
			...
			if(
				...
				config.pierce_rating == Config::PierceRating::max_target_minus_source_hop_dist_with_closer_dist_tie_break ||
				config.pierce_rating == Config::PierceRating::max_target_minus_source_hop_dist_with_source_dist_tie_break ||
			...
			case Config::GraphSearchAlgorithm::pseudo_depth_first_search:
				cutter.init(graph, PseudoDepthFirstSearch(), PierceNodeScore(config), dist_type, p, random_seed, config.skip_non_maximum_sides == Config::SkipNonMaximumSides::skip, parallel);
				break;
			
			case Config::GraphSearchAlgorithm::breadth_first_search:
				cutter.init(graph, BreadthFirstSearch(), PierceNodeScore(config), dist_type, p, random_seed, config.skip_non_maximum_sides == Config::SkipNonMaximumSides::skip, parallel);
				break;
		...
		void enum_cuts(const ShallContinue &shall_continue, const ReportCut &report_cut, const bool) {
			do {
				report_cut(*this, get_current_cutter_id());
			} while (shall_continue(*this) && advance());
		}
		...
		bool advance(){
			...
			switch(config.graph_search_algorithm){
			case Config::GraphSearchAlgorithm::pseudo_depth_first_search:
				return cutter.advance(graph, PseudoDepthFirstSearch(), PierceNodeScore(config), config.skip_non_maximum_sides == Config::SkipNonMaximumSides::skip);
			
			case Config::GraphSearchAlgorithm::breadth_first_search:
				return cutter.advance(graph, BreadthFirstSearch(), PierceNodeScore(config), config.skip_non_maximum_sides == Config::SkipNonMaximumSides::skip);
		...
		int get_current_flow_intensity() const {
			return cutter.get_current_cut().size();
		}
	...
	class CutterFactory {
	public:
		explicit CutterFactory(const Config& config) : config(config) {}
